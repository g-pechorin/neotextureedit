...
class Matrix3x3ParameterEditor extends AbstractParameterEditor implements DocumentListener, ActionListener {
	...
	public Matrix3x3ParameterEditor(Matrix3x3Param matrixParam) {
		...
		x = startX;
		zoomInc = matTransformButton("z+", x, y);
		x += MAT_BUT_WIDTH;
		tyDec = matTransformButton("y-", x, y);
		x += MAT_BUT_WIDTH;
		zoomDec = matTransformButton("z-", x, y);
		x += MAT_BUT_WIDTH;
		x = startX;
		y += h;
		txDec = matTransformButton("x-", x, y);
		x += MAT_BUT_WIDTH;
		resetmat = matTransformButton("I", x, y);
		x += MAT_BUT_WIDTH;
		txInc = matTransformButton("x+", x, y);
		x += MAT_BUT_WIDTH;
		x = startX;
		y += h;
		rotDec = matTransformButton("r-", x, y);
		x += MAT_BUT_WIDTH;
		tyInc = matTransformButton("y+", x, y);
		x += MAT_BUT_WIDTH;
		rotInc = matTransformButton("r+", x, y);
		x += MAT_BUT_WIDTH;
		x = startX;
		y += h;
	}
